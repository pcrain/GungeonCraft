...
public static class CwaffConfig
{
  ...
  public static void Init()
  {
    ...
    for (int i = 0; i < 3; ++i)
    {
      Gunfig.AddToggle(key: "testCheck", label: "Hello there! :D".Cyan(), enabled: false, callback: (_, newVal) => ETGModConsole.Log($"it worked O: {(newVal == "1" ? "on" : "off")}") );
      Gunfig.AddLabel("A Label *O*".Magenta());
      ...
      //   callback: (_, newVal) => ETGModConsole.Log($"toggled to: {newVal}"));
      Gunfig.AddScrollBox(key: "testScroll", label: "Another Line Test!".Red(), options: new(){"one".Yellow(), "two"}, info: new(){"one line".Green(), "still one line"},
        callback: (_, newVal) => ETGModConsole.Log($"toggled to: {newVal}"));
      Gunfig.AddScrollBox(key: "testScroll2", label: "Last Line Test!".Blue(), options: new(){"one", "two"},
        callback: (_, newVal) => ETGModConsole.Log($"toggled to: {newVal}"));
