...
public class Groundhog : CwaffGun
{
    ...
    public static void Init()
    {
        Lazy.SetupGun<Groundhog>(ItemName, ShortDescription, LongDescription, Lore)
          ...
          .InitSpecialProjectile<GroundhogProjectile>(GunData.New(sprite: null, clipSize: 1, cooldown: 0.75f, shootStyle: ShootStyle.Charged, chargeTime: 2f, hideAmmo: true,
    ...
    {
        protected override bool OnFreeFired()
        {
          const float MAX_FREE_FIRE_RANGE = 16f; // free fired projectiles should have less range
          this.baseData.range = Mathf.Min(this.baseData.range, MAX_FREE_FIRE_RANGE);
          return true;
        }
