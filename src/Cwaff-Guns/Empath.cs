...
public class Empath : CwaffGun
{
    ...
    public static void Init()
    {
        Lazy.SetupGun<Empath>(ItemName, ShortDescription, LongDescription, Lore)
          ...
          .InitSpecialProjectile<EmpathProjectile>(GunData.New(sprite: "empath_projectile", clipSize: 15, cooldown: 0.13f, shootStyle: ShootStyle.SemiAutomatic, fps: 11,
    ...
    {
        if (frame == 7 && clip.name == "empath_reload")
            CwaffVFX.Spawn(_PsychicVFX, position: this.gun.sprite.WorldCenter + (base.transform.rotation * (new Vector2(-6f / 16f, -2f / 16f))).XY(),
              lifetime: 0.5f, startScale: 0.5f, endScale: 3.0f, fadeOutTime: 0.4f, anchorTransform: base.transform);
    }
}

public class EmpathProjectile : Projectile
{
    ...
    {
        base.Start();
        this._mastered = this.Mastered<Empath>();
        base.specRigidbody.OnPreRigidbodyCollision += this.EmpathProjPreCollision;
        base.specRigidbody.OnRigidbodyCollision += this.EmpathProjCollision;
    }

    private void EmpathProjPreCollision(SpeculativeRigidbody myRigidbody, PixelCollider myPixelCollider, SpeculativeRigidbody otherRigidbody, PixelCollider otherPixelCollider)
    {
        if (!otherRigidbody)
        ...
        else if (this._mastered && otherRigidbody.gameObject.GetComponent<AIActor>())
            return;
        PhysicsEngine.SkipCollision = true;
    }

    private void EmpathProjCollision(CollisionData collision)
    {
        if (!collision.OtherRigidbody || collision.OtherRigidbody.gameObject is not GameObject otherObject)
            return;
        if (this._mastered && otherObject.GetComponent<AIActor>() is AIActor target)
        {
        ...
        CwaffVFX.SpawnBurst(prefab: Empath._PsychicVFX, numToSpawn: 4, basePosition: enemyPos, positionVariance: 1f,
