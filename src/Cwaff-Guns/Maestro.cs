...
public class Maestro : CwaffGun
{
    ...
    private void RedirectProjectiles(AIActor targetEnemy)
    {
        ...
        foreach (Projectile p in StaticReferenceManager.AllProjectiles)
        {
            ...
            delay += baseDelay;
            baseDelay = Mathf.Max(0.016f, baseDelay - 0.005f);
    ...
    {
        if (!p.GetComponent<MaestroProjectile>())
            p.AddComponent<MaestroProjectile>().Setup();
    }

    private static void DeregisterProjectileHost(Projectile p)
    {
        if (p.GetComponent<MaestroProjectile>() is MaestroProjectile mp)
            mp.Teardown();
    }
}

public class MaestroProjectile : MonoBehaviour
{
...
public class MaestroHost : MonoBehaviour
{
    // private void Start()
    // {
    //     ETGModConsole.Log($"new Maestro Host {base.GetComponent<AIActor>().ActorName}");
    // }
}
